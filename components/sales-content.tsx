...
import type { PixTransaction } from "@/lib/adm"
...
// Data curta + "há X dias", espelhando o padrao da referencia.
function formatDate(iso: string): { full: string; rel: string } {
  ...
  const rel =
    diffDays <= 0 ? "hoje" : diffDays === 1 ? "há 1 dia" : `há ${diffDays} dias`
  return { full, rel }
}
...
export function SalesContent() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!active) return
      if (!error && data) setSales(data as PixTransaction[])
      setLoading(false)
    })()
    return () => {
      active = false
  ...
  return (
    <div className="animate-fade-up">
      ...
      {/* O PIX Aprovado ocupa a largura toda (destaque maior), Pendente e
          Ticket medio dividem a linha de baixo. */}
