  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (data.session?.status === "running") {
        ...
        // Durante a fase "settling" precisamos continuar cutucando o servidor
        // para que ele consulte o resultado da transacao na corretora — mesmo
        // que tradeEndsAt esteja no futuro (deadline da janela de liquidacao).
        // Sem isso, a confirmacao so avancaria pelo cron (1x/min) e demoraria.
        const settling = data.session.phase === "settling"
        if (due || settling) {
